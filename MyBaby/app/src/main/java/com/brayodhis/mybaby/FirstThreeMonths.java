...
public class FirstThreeMonths extends Activity {
TextView first;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_first_three_months);
        first=(TextView)findViewById(R.id.tvfirstChi);
        Typeface tf=Typeface.createFromAsset(getAssets(),"fonte.ttf");
        first.setTypeface(tf);

        try {
            InputStream is = getAssets().open("yfirst.txt");
            ...
            first.setText(text);
            first.setText(Html.fromHtml(text));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        {
            String lyrics=first.getText().toString();
